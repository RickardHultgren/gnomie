...
#								res_box.height += res_rev.height
	...
	def edit_post(self, pop_item, *args):
		...
		res_bttn.bind(on_release=partial(self.edit_nomen, pop_item, res_inpt))
	...
	def edit_nomen(self, pop_item, res, *args):
		...
		think_things.put("%s"%pop_item, title=res, state=self.state_claim, nomen=preNomen)
		think_things_cpy["%s"%pop_item]={"title":res, "state":self.state_claim, "nomen":preNomen}
		self.planupdate()
...
#		self.planupdate()
	...
	def add_nomen(self, preNomen, res, *args):
		...
		keylist = []
		for key in think_things_cpy.keys():
		...
		for h in range(0,length) :
			if keylist[h] > h:
				...
				for j in ["title","state","nomen"] :
					if j == "title" :
						title_var = str(think_things_cpy["%s"%(keylist[h])][j])
					if j == "state" :
						state_var = str(think_things_cpy["%s"%(keylist[h])][j])
					if j == "nomen" :
						nomen_var = str(think_things_cpy["%s"%(keylist[h])][j])
				think_things.delete("%s"%str(keylist[h]))
				think_things_cpy.pop("%s"%str(keylist[h]), None)
